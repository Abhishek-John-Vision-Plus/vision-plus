...
import { Plus, Trash2, Edit2, Save, X, ChevronDown, ChevronUp, Search, Filter } from "lucide-react";
...
export default function ManageQuestionsPage() {
  ...
  // Get current user's role and allowed process
  const isSuperAdmin = user?.role === 'SUPER_ADMIN' || user?.role === 'ADMIN'; // Assuming ADMIN is super for now, or use specific role
  const allowedProcess = user?.process || "";

  useEffect(() => {
    if (user) {
      fetchQuestions();
      if (!isSuperAdmin && allowedProcess) {
        setNewQuestion(prev => ({ ...prev, process: allowedProcess }));
        setFilterProcess(allowedProcess);
      }
    }
  }, [user]);
